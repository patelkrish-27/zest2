...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      watch: { ignored: ["**/.figma/**"] },
      proxy: {
        // Backend proxy — Vite forwards /api to Express (src/backend/server.ts on :3001)
        // Falls back gracefully if backend not running (frontend uses local generatePrompt)
        "/api": {
